...
float world(vec3 p)
{
    ...
    float c = max(blend(blend(cy, cz, 0.25), c3, 0.25), box(p, vec3(cos(T) * 0.25 + 1.25, 1.5, 1.5)));

    float sx = sphere(translate(p, vec3(cos(T), 0.25, 0.25)), 0.25);
    float sy = sphere(translate(p, vec3(0.25, cos(T), 0.25)), 0.25);
    float sz = sphere(translate(p, vec3(0.25, 0.25, cos(T))), 0.25);
